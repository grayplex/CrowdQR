...
pm.test(
  "Response contains expected user fields with matching username",
  function () {
    const responseJson = pm.response.json();
    ...
    const urlPathParts = pm.request.url.path;
    const usernameIndex =
      urlPathParts.findIndex((part) => part === "username") + 1;
    const requestedUsername = urlPathParts[usernameIndex];

    pm.expect(responseJson).to.have.property("userId");
    pm.expect(responseJson).to.have.property("username");
    pm.expect(responseJson.username.toLowerCase()).to.equal(
...
pm.test(
  "Response contains expected event fields with matching slug",
  function () {
    const responseJson = pm.response.json();
...
pm.test(
  "Response is an array of requests for the requested event",
  function () {
    const responseJson = pm.response.json();
...
pm.test("Response is an array of votes for the requested request", function () {
  ...
  const requestIdIndex =
    urlPathParts.findIndex((part) => part === "request") + 1;
...
pm.test(
  "Response is an array of sessions for the requested event",
  function () {
    const responseJson = pm.response.json();
...
pm.test(
  "Response contains session for the requested event and user",
  function () {
    const responseJson = pm.response.json();
...
pm.test("Response is an array of requests sorted by vote count", function () {
  ...
  if (responseJson.length > 1) {
    for (let i = 0; i < responseJson.length - 1; i++) {
      pm.expect(responseJson[i].voteCount >= responseJson[i + 1].voteCount).to
        .be.true;
    }
  }
});
