...
document.addEventListener("DOMContentLoaded", function () {
      ...
      function (event) {
        ...
        if (firstInvalid) {
          ...
          setTimeout(() => {
            window.scrollTo({
              top:
                firstInvalid.getBoundingClientRect().top + window.scrollY - 100,
              behavior: "smooth",
            });
          }, 100);
        }
  ...
  const songNameInputs = document.querySelectorAll(
    'input[name="NewSongRequest.SongName"]',
  );
  songNameInputs.forEach((input) => {
    if (!input.hasAttribute("pattern")) {
      ...
      input.setAttribute(
        "title",
        "Song name must be between 2 and 255 characters",
      );
    }
  });
});
